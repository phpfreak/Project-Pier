...
<?php if (is_array($users = $company->getUsersOnProject($project)) && count($users)) { ?>
  <div class="companyDetails">
    <div class="companyName"><?php echo checkbox_field($post_data_name.'[notify_company_' . $company->getId() . ']', array_var($post_data, 'notify_company_' . $company->getId()), array('id' => 'notifyCompany' . $company->getId() )) ?> <label for="notifyCompany<?php echo $company->getId() ?>" class="checkbox"><?php echo clean($company->getName()) ?></label></div>
