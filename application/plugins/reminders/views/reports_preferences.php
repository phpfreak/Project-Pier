<?php
  ...
  if ($user->getId() == logged_user()->getId()) {
    set_page_title("Reports and Reminders");
    account_tabbed_navigation();
    account_crumbs("Reports and Reminders");
  } else {
...
?>  
<form action="<?php echo get_url('reports', 'update_prefs') ?>" method="post">
      <fieldset>
     ...
     <fieldset>
        ...
        <div>
    	    ...
    	    <?php echo yes_no_widget('prefs_form[reports_enabled]', 'reportrFormEnableDaily', $reminder_prefs->getReportsEnabled(), lang('yes'), lang('no')) ?>
          	<?php echo label_tag(lang('summarize by', null, false)) ?>
			<?php echo yes_no_widget('prefs_form[reports_summarized]', 'reportsSummarizedBy', $reminder_prefs->getReportsSummarizedBy(), lang('project'), lang('all'));?>
			<?php echo label_tag(lang('include whom question'), null, false) ?>
