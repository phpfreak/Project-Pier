...
// Ver:		$Id: jpgraph_gantt.php,v 1.4.10.1 2006/02/28 19:46:57 gregorerhardt Exp $
//
...
DEFINE('ACTYPE_GROUPMILESTONE',3);
...
// PUBLIC METHODS
	...
	for( $i=0; $i < $num; ++$i) {
		...
		case ACTYPE_GROUPMILESTONE: //Modif for projectpier
		    // Create a slightly smaller height bar since the
		    // "wings" at the end will make it look taller
		    $a = new GanttBar($data[$i][0],$data[$i][2],$data[$i][3],$data[$i][4],'',8);
		    $a->title->SetFont($this->iSimpleFont,FS_BOLD,$this->iSimpleFontSize);		
		    $a->rightMark->Show();
		    //$a->rightMark->SetType(MARK_RIGHTTRIANGLE);
		    $a->rightMark->SetType(MARK_DIAMOND);
		    $a->rightMark->SetWidth(8);
		    $a->rightMark->SetColor('red');
		    $a->rightMark->SetFillColor('red');
...
//("Unknown formatting style for week.");
	...
	$sn = utf8_decode($this->iDateLocale->GetShortMonthName($aMonthNbr));
	$ln = utf8_decode($this->iDateLocale->GetLongMonthName($aMonthNbr));
...
// CONSTRUCTOR	
	...
	//$this->mark->SetType(MARK_RIGHTTRIANGLE);
